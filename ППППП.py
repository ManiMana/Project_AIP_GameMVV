...
class VirtualVirusApp:
    ...
    def load_data(self):
        """Загрузка данных игрока из файла.
            ...
            Эта функция загружает данные игрока из файла `player_data.json`.
            Если файл существует, данные считываются и присваиваются соответствующим
            атрибутам игрока. Если файл не найден, вызывается экран создания аккаунта.

            Атрибуты, загружаемые из файла:
                player_name (str): Имя игрока. По умолчанию "Игрок".
                player_level (int): Уровень игрока. По умолчанию 1.
                player_points (int): Очки игрока. По умолчанию значение INITIAL_POINTS.
                stats (dict): Статистика игрока. По умолчанию все статусы равны 50.
                last_update_time (float): Время последнего обновления в формате timestamp.

            Если с момента последнего обновления прошло время,
            применяется эффект времени через метод `apply_time_effect`.

            Если файл не существует, вызывается метод `create_account_screen`.

            Raises:
                FileNotFoundError: Если файл `player_data.json` не найден (необязательно,
                так как обрабатывается условием if).
    ...
    def load_image(self, filename, size=None):
        """Загрузка изображения с обработкой ошибок.
            ...
            Эта функция загружает изображение из указанного файла.
            Если указаны размеры, изображение будет масштабировано до этих размеров.
            В случае возникновения ошибки при загрузке изображения,
            выводится сообщение об ошибке, и функция возвращает None.

            Args:
                filename (str): Путь к файлу изображения, который необходимо загрузить.
                size (tuple, optional): Кортеж из двух целых чисел (ширина, высота)
                    для изменения размера изображения. Если не указано, изображение
                    загружается в оригинальном размере.

            Returns:
                pygame.Surface or None: Загруженное изображение в виде объекта
                pygame.Surface, или None, если загрузка не удалась.

            Raises:
                pygame.error: Исключение, возникающее при ошибке загрузки изображения,
                которое обрабатывается внутри функции.
...
def create_new_strawberry():
    ...
    """
    return Strawberry(random.randint(0, 1440 - 30), 0)
...
def create_new_mushroom():
    ...
    """
    return Mushroom(random.randint(0, 1440 - 30), 0)
...
def create_new_oclock():
    ...
    """
    return Oclock(random.randint(0, 1440 - 30), 0)
