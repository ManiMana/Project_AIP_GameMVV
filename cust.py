...
class VirtualVirusApp:
    ...
    def update_stat(self, stat, value):
        ...
        if self.player_points >= cost:  # Проверка, достаточно ли очков
            ...
            self.last_update_time = pygame.time.get_ticks()  # Обновляем время последнего обновления
...
class Strawberry:
    ...
    def move(self):
        if self.visible:
            ...
            if self.rect.y > 810:  # Если выходит за пределы экрана
                self.rect.y = 0
                self.rect.x = random.randint(0, 1440 - self.rect.width)
...
class Player:
    def __init__(self):
        ...
        self.rect = self.image.get_rect(topleft=(720, 600))  # Начальная позиция
        self.speed = 10
    ...
    def move(self):
        ...
        if self.rect.x < 0:
            self.rect.x = 0
        if self.rect.x > 1440 - self.rect.width:
...
# Основная функция игры "Падающая клубника"
def run_game():
    pygame.init()
    screen_width, screen_height = 1440, 810
    ...
    while running:
        ...
        # Инициализация текстов
        text_score = score_screen_text.render(str(score), True, (255, 255, 255))  # Счет на экране
        text_rect_score = text_score.get_rect(center=(100, 100))

        text_time = score_screen_text.render(f'{(duration - (current_time - start_time)) // 1000} сек', True,
                                              (255, 255, 255))  # Счетчик времени
...
# Прорисовка заднего фона
        ...
        if current_time - start_time < duration:
            ...
            # Время вышло, показываем итоговый счет
            end_text = score_screen_text.render('Время вышло!', True, (255, 0, 0))
            end_rect = end_text.get_rect(center=(screen_width // 2, screen_height // 2 - 100))
            score_text = score_screen_text.render(f'Ваш счёт: {score * 5}', True, (255, 255, 255))
            score_rect = score_text.get_rect(center=(screen_width // 2, screen_height // 2))
            menu_text = score_screen_text.render("Нажмите 'M' для возврата в меню", True, (255, 255, 255))
            ...
            # Проверка нажатия кнопки для возврата в меню
            keys = pygame.key.get_pressed()
            if keys[pygame.K_m]:
...
# Основная функция игры "Пятнашки"
...
# Функции для сохранения и загрузки денег
